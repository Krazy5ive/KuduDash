...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .kd-role-btn:hover { background: #1a1b1b; color: #ccc; }
  .kd-role-btn.active { background: #e8a045; color: #0e0f0f; }
  ...
  .kd-nav-icon { width: 16px; height: 16px; flex-shrink: 0; }
  ...
  .kd-progress-ring { position: relative; width: 80px; height: 80px; }
  .kd-progress-ring svg { transform: rotate(-90deg); }
...
function StudentDashboard() {
  const [menuToggles, setMenuToggles] = useState([true, true, false, true]);
  const orders = [
      ...
      <div className="kd-grid2">
        ...
        <div className="kd-card kd-full">
          ...
          <table className="kd-table">
            <thead>
              <tr>
                <th>Item</th><th>Vendor</th><th>Date</th><th>Amount</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {[
    ...
    <>
      ...
      <div className="kd-app">
        <div className="kd-sidebar">
          ...
          <div className="kd-role-switcher">
            <div className="kd-role-label">View as</div>
            {["student", "vendor", "admin"].map(r => (
              <button key={r} className={`kd-role-btn ${role === r ? "active" : ""}`} onClick={() => { setRole(r); setActiveNav(0); }}>
          ...
          <nav className="kd-nav">
            {NAV[role].map((item, i) => (
              <div key={i} className={`kd-nav-item ${activeNav === i ? "active" : ""}`} onClick={() => setActiveNav(i)}>
